...
### Adding a workflow
On your `shr-private` repository, use the GitLab interface to add the following new file [`.gitlab-ci.yml`](https://docs.gitlab.com/ee/ci/quick_start/#create-a-gitlab-ciyml-file). This workflow contains a single job that runs on a __self-hosted runner__ called "self-hosted" with 3 steps to build the projects in the sequence `library`, `componentA` and `componentB`, after the repository is checked out:
```yaml
variables: 
  BUILD_TYPE: Debug
